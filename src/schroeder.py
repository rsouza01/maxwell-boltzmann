...
def to_rounded_float(value: float) -> float:
...
def calc(name: str, T: float, P: float, Z_int: float, m: int, epsilon_ev: float) -> float:
    ...
    # l_Q = lQ(T, m)
    # v_Q = l_Q ** 3
    l_Q = to_rounded_float(lQ(T, m))
    v_Q = to_rounded_float(l_Q**3)
    

    print(f"lQ = {l_Q}, vQ = {v_Q}")
    _mu = to_rounded_float(mu(T, Z_int, P, v_Q))
    mu_ev = to_rounded_float(_mu/scc.electron_volt)
    ...
    diff = round(epsilon_ev - mu_ev, 1)
    print(f"diff = {diff} eV")

    gibbs = math.exp(-diff/(k_eV * T))
    print(f"gibbs = {gibbs}")
